...
export default function ProductDetailPage() {
  ...
  const moq = pick(product.moq).trim();
      ...
      <div className="section-shell py-12 md:py-16">
        <div className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <div className="ff-card p-7 md:p-10">
            ...
            {showDescription && <p className="mt-5 text-lg leading-8 text-slate-600">{description}</p>}
            {product.variants.length > 1 && <label className="mt-7 block"><span className="admin-label">{locale === "zh" ? "规格" : "Specification"}</span><select value={selectedVariant?.id || ""} onChange={(event) => setSelectedVariantId(event.target.value)} className="admin-input mt-2">{product.variants.filter((item) => item.active).map((variant: ProductVariant) => <option key={variant.id} value={variant.id}>{variant.specification}</option>)}</select></label>}
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {(packing || weight) && <Info label={locale === "zh" ? "包装 / 规格" : "Packing / Specification"} value={packing || weight} />}
              {showWeight && <Info label={locale === "zh" ? "重量" : "Weight"} value={weight} />}
              {moq && <Info label={locale === "zh" ? "最低订购量" : "Minimum Order"} value={moq} />}
            </div>
