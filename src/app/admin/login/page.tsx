...
export default function AdminLoginPage() {
  ...
  const [email, setEmail] = useState("admin@famfood.local");
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (supabase) {
        ...
        if (profilePayload.profile?.role !== "admin") throw new Error("This account is not an admin.");
        window.localStorage.setItem("famfood-session", JSON.stringify({ role: "admin", email, token: data.session.access_token }));
        router.push("/admin");
        return;
      }
      ...
      setError("Use admin@famfood.local / admin123 for local demo, or configure Supabase Auth.");
    } catch (caught) {
  ...
  return (
    <div className="grid min-h-screen place-items-center bg-[#043f4f] px-4 py-20">
      <form onSubmit={submit} className="w-full max-w-md bg-white p-8 shadow-2xl">
        ...
        <h1 className="display-serif mt-5 text-4xl font-medium text-slate-950">Admin Login</h1>
        <p className="mt-2 text-sm leading-6 text-slate-600">Demo login: admin@famfood.local / admin123</p>
